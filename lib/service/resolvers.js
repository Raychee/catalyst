    ...
    makeResolvers({auth = () => {}} = {}) {
        ...
        const updateFns = {
            ...
            Job: async (context, query, input, others) => {
                ...
                if (waitOptions) {
                    if (!(waitOptions.until && waitOptions.until.length > 0)) {
                        throw new UserInputError(`wait.until must be specified with at least one job's status`);
                    }
                    if (!(waitOptions.timeout >= 0 && waitOptions.timeout <= 3600)) {
                        throw new UserInputError(`wait.timeout must be >= 0 and <= 3600`);
                    }
                    if (!(waitOptions.pollInterval > 0 && waitOptions.pollInterval <= waitOptions.timeout)) {
                        throw new UserInputError(`wait.pollInterval must be >= 0 and <= wait.timeout (which is ${waitOptions.timeout})`);
                    }
                }
                let {id, ...update} = input;
                let job;
                ...
                if (job) {
                    while (true) {
                        if (waitOptions && waitOptions.timeout > 0) {
                            if (!waitOptions.until.includes(job.status)) {
                                job = await waitForJob(context, job._id, waitOptions);
                            }
                        }
        ...
        async function waitForJob(context, jobId, waitOptions) {
            setTimeout(async () => {
            ...
            return await context.jobWatcher.watch(
                jobId,
                {
                    pipeline: [{
                        $match: {
