...
const {isEqual, shrink, diff} = require('@raychee/utils');
const {
...
class Operations {
    ...
    async computeJob(job) {
        ...
        const {_id, id, full, lockedWrite, lockedBy, params: params1, context: context1, ...rest} = job;
        return {
            ...shrink({
                _id: _id || id,
    ...
    async insertJob(input) {
        ...
        if (full.dedupRecent) {
            ...
            const {it, close} = await this._findDuplicateJobs(full) || {};
            if (it) {
                for await (const {local, ...dup} of it) {
                    this.logger.info(
                        'Job scheduling is skipped as a duplicate of job ', dup._id,
                        ': ', {domain, type, params}
                    );
                    duplicate = dup;
                    if (dup.status === 'SUCCESS') break;
                }
                await close();
            }
            if (duplicate) {
    ...
    async _findDuplicateJobs({_id, domain, type, params, dedupWithin, timeCreated = new Date()}) {
        if (dedupWithin < 0) {
            return;
        }
        const dupQuery = {
            ...
            $and: [
                {
                    $or: [
                        {status: {$in: ['PENDING', 'RUNNING', 'DELAYED']}},
        ...
        if (dupQueryParams.length > 1) {
            dupQuery.$and.push({$or: dupQueryParams});
        } else if (dupQueryParams.length === 1) {
            dupQuery.$and.push(dupQueryParams[0]);
        }
        const cursor = this.jobs.find(dupQuery);
        let it;
        if (taskType.dedup) {
            it = cursor;
        } else {
