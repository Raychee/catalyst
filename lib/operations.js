...
class Operations {
    ...
    constructor(logger, mongodb, taskLoader) {
        ...
        this.taskTypes = mongodb.collection(COLLECTION_NAMES.type);
     ...
     */
    async computeTaskDomainConfig(domain) {
    ...
    async ensureTaskDomainConfig(domainName) {
        const domain = await this.computeTaskDomainConfig({domain: domainName});
        const now = new Date();
    ...
    async updateTaskDomainConfigs(filter, update) {
        if (isEmpty(update)) return 0;
        const nonNull = mapValues(
            pickBy(update, (v, p) => p !== 'lockedBy' && isNil(v)),
            (v, p) => DEFAULT_TASK_DOMAIN_CONFIG[p]
        );
        const full = {...nonNull, mtime: new Date()};
        const modified = await this._fullUpdateMany(this.domains, filter, update, full);
        const domains = await this.domains.find(filter).toArray();
        const {maxConcurrency, ...taskTypeUpdate} = {...update, ...nonNull};
        await this.updateTaskTypeConfigs(
     ...
     * @return {Promise<object>}
     */
    async computeTaskTypeConfig(domain, type) {
        ...
        domain = await this.computeTaskDomainConfig(domainName);
        if (!type) {
            type = await this.taskTypes.findOne({domain: domainName, type: typeName}) || {};
        }
    ...
    async ensureTaskTypeConfig(domainName, typeName) {
        const taskType = await this.computeTaskTypeConfig({domain: domainName, type: typeName});
        const now = new Date();
        const {value: {_id, ...ret} = {}} = await this.taskTypes.findOneAndUpdate(
    ...
    async updateTaskTypeConfigs(filter, update, {overwrite = true} = {}) {
        if (isEmpty(update)) return;
        const taskTypes = await this.taskTypes.find(filter).toArray();
        const complexUpdate = pickBy(update, (v, p) =>
            isNil(v) && TASK_TYPE_INHERITED_PROPERTIES.includes(p)
        );
        const now = new Date();
        let modified = 0;
        if (overwrite) {
            const simpleUpdate = omit(update, Object.keys(complexUpdate));
            if (!isEmpty(simpleUpdate)) {
                modified = await this._fullUpdateMany(this.taskTypes, filter, simpleUpdate, {mtime: now});
            ...
            if (!isEmpty(complexUpdate)) {
                for (let taskType of taskTypes) {
                    ...
                    taskType = await this.computeTaskTypeConfig(shrink({domain, type, ...taskType.local, ...update}));
                    const nonNull = pick(taskType, Object.keys(complexUpdate));
                    const full = {...nonNull, mtime: now};
                    const updated = await this._fullUpdateOne(this.taskTypes, {domain, type}, complexUpdate, full);
                    if (updated) modified++;
            ...
            if (Object.values(update).some(isNil)) {
            ...
            for (const [p, v] of Object.entries(update)) {
                const targets = await this.taskTypes.find({...filter, [`local.${p}`]: null}).toArray();
                const {matchedCount} = await this.taskTypes.updateMany({
                    ...filter,
                    [`local.${p}`]: null
                }, {$set: {[p]: v}});
    ...
    async computeTask(task) {
        ...
        const taskTypeConfig = await this.computeTaskTypeConfig(domain, type);
    ...
    async updateTasks(filter, update, {overwrite = true} = {}) {
        ...
        for await (const tasks of this._findInBatch(this.tasks, filter)) {
            let complexUpdate = pickBy(update, (v, p) =>
            ...
            if (overwrite) {
                ...
                const {subTasks: updateSubTasks, ...updateRest} = update;
    ...
    async computeJob(job) {
        ...
        const taskType = await this.computeTaskTypeConfig(job.domain, job.type);
