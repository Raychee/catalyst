...
const {isEqual, shrink, diff, interpolate} = require('@raychee/utils');
const {
...
class Operations {
    ...
    async computeTask(task) {
        ...
        return {
            ...shrink({
                domain, type,
                enabled: true, nextTime: new Date(0),
                ...rest0, ...rest1,
    ...
    async updateTasks(filter, update, {overwrite = true} = {}) {
        ...
        if (['mode', 'interval', 'schedule'].some(p => p in _update)) {
            await this._validateTask(_update);
        }
        if (_update.interval || _update.schedule) {
            _update.nextTime = new Date(0);
        }
    ...
    async computeJob(job) {
        ...
        let enabled = true, params = undefined, context = undefined, task = undefined;
        if (job.task) {
        ...
        const {_id, id, full, lockedBy, params: params1, context: context1, ...rest} = job;
        const now = new Date();
        const computed = {
            ...shrink({
                _id: _id || id,
                status: 'PENDING', trials: [], timeCreated: now, timeScheduled: now,
                ...taskTypeScheduling, ...taskScheduling, ...subTaskScheduling,
                ...shrink(rest),
                ...(!enabled ? {status: 'CANCELED'} : undefined)
            }),
            params: params1 || params || {}, context: context1 || context || {},
        };
        ...
        if (full.dedupRecent) {
            ...
            if (dups.length >= dedupLimit) {
                for (const dup of dups.slice(0, dups.length - dedupLimit + 1)) {
                    ...
                    if (!['PENDING', 'DELAYED', 'RUNNING'].includes(dup.status)) continue;
    ...
    async updateJobs(filter, update, {overwrite = true, force = false} = {}) {
        ...
        if (['task'].some(p => p in _update)) {
            await this._validateJob(_update);
        }
        const complexUpdate = pickBy(_update, (v, p) =>
    ...
    _renderJobParams(job, task) {
        const env = {
            task: task ? {
                id: job.task ? job.task.toString() : null,
                mode: task.mode,
                interval: task.interval,
                schedule: task.schedule,
                timezone: task.timezone,
            } : {},
            timeScheduled: job.timeScheduled.toISOString(),
            timeCreated: job.timeCreated.toISOString(),
        };
        const map = v => {
            if (isPlainObject(v)) {
                return mapValues(v, map);
            } else if (Array.isArray(v)) {
                return v.map(vv => mapValues(vv, map));
            } else if (typeof v === 'string') {
                return interpolate(v, env, {defaultValue: null});
            } else {
                return v;
            }
        };
        job.params = mapValues(job.params, map);
    }
