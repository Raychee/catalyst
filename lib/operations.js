...
class Operations {
    ...
    async computeTask(task) {
        ...
        const computed = {
            domain, type,
            enabled: true, params: {}, context: {}, nextTime: new Date(0),
            ...rest0, ...rest1,
            subTasks: this._mergeSubTaskConfigs(subTasks0, subTasks1)
        };
        const {params, context, ...rest2} = computed;
        return {...shrink(rest2), params, context};
    }
    ...
    async insertJob(input) {
        ...
        for await (const {local, ...dup} of this._findDuplicateJobs(full)) {
            if (full.dedupRecent) {
                this.logger.info(
    ...
    async* _findDuplicateJobs({domain, type, params, dedupWithin, timeCreated = new Date()}) {
        ...
        if (taskType.dedup) {
            ...
            for await (const dup of this.jobs.find(dupQuery)) {
                if (deepEqual(params, dup.params || {})) {
                    yield dup;
                }
            }
        }
