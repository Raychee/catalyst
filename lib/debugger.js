...
const {JobWatcher} = require('./watcher');
...
module.exports = class extends Runnable {
    ...
    constructor(options) {
        ...
        /** @type {JobWatcher} */
        this.jobWatcher = new JobWatcher(undefined, {targetStatus: ['SUCCESS', 'FAILED', 'CANCELED']});
                ...
                job.on('wait', (jobId) => {
                    for (const j of [...succeeded, ...failed]) {
                        if (j.config._id.toString() === jobId.toString()) {
                            job._jobWatcher.watched(jobId, j.config);
                        }
                    }
                });
                running.push({
                    ...
                    promise: job._execute()
                        ...
                        .finally(() => {
                            ...
                            for (const {job: j} of running) {
                                j._jobWatcher.watched(jobId, job.config);
                            }
                            enqueue();
                        })
                });
    ...
    async spawnJob(jobConfig, taskTypeSpec) {
        ...
        const job = new Job(jobConfig, taskType, this.jobWatcher);
