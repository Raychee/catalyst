...
module.exports = class extends Runnable {
    ...
    async run({signal}) {
        ...
        if (this.options.debugger.jobs.length > 0) {
            let stop = false;
            signal.then(() => stop = true);
            const pending = [...this.options.debugger.jobs];
            let running = 0, complete = 0;
            const queue = [];
            const enqueue = () => {
                if (stop) return;
                        ...
                        this.spawn(job)
                            .then(job => {
                                if (this.options.debugger.includeScheduledJobs) {
                                    pending.push(...job._scheduledJobs);
                                }
                            })
            ...
            while (pending.length > 0 && running < this.options.debugger.concurrency) {
                enqueue();
            }
            let total = queue.length;
            while (true) {
                await Promise.all(queue);
                if (queue.length === total || stop) break;
                total = queue.length;
            }
    ...
    async spawn(jobConfig, taskTypeSpec) {
        ...
        process.on('SIGTERM', () => job._interrupted = true);
        process.on('SIGINT', () => job._interrupted = true);
        await job._execute();
        return job;
    }
