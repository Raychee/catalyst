...
class Browser {
    ...
    async newPage(logger, {incognito = true, filterRequests} = {}) {
        logger = logger || this.logger;
        let page = undefined, pageState = undefined,
            lastPageGoto = undefined, lastPageGotoOptions = undefined,
        ...
        const makePage = dedup(async () => {
            ...
            if (pageState && this.loadPageStateFn) {
                ...
                return;
            } else if (lastPageGoto) {
                const newPage = await this.newPage(logger, {incognito, filterRequests});
                if (defaultTimeout) page.setDefaultTimeout(defaultTimeout);
                if (defaultNavigationTimeout) page.setDefaultNavigationTimeout(defaultNavigationTimeout);
                await newPage.goto(lastPageGoto, lastPageGotoOptions);
                page = newPage._rawPage;
            }
