...
const {JobRuntime} = require('../error');
...
class Browser {
    ...
    async newPage(logger, {incognito = true, filterRequests} = {}) {
        logger = logger || this.logger;
        let page = undefined, pageState = undefined, loadingPageState = false,
        ...
        const makePage = dedup(async () => {
            ...
            if (pageState && this.loadPageStateFn && !loadingPageState) {
                loadingPageState = true;
                for (let trial = 1; trial <= this.maxRetryIdentities; trial++) {
                    try {
                        await this.loadPageStateFn.call(logger, makeProxy(), pageState);
                        break;
                    } catch (e) {
                        if (e instanceof JobRuntime) {
                ...
                const promises = invokes.map(({method, args = []}) => {
                    ...
                    return fn.call(this_, ...args).then(
                        (returned) => {
                            ...
                            if (method === 'goto') {
                                [lastPageGoto, lastPageGotoOptions] = args;
                            }
