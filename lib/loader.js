...
class TaskLoader {
    ...
    async load({verbose = false} = {}) {
        ...
        this.loadedTaskDomains = {};
        this.loadedTaskTypes = {};
        logger.print('Loading task domains... ');
        ...
        logger.reprintln('Loading task domains... Done.');
        logger.print('Loading task types... ');
        for await (let scanned of this._scan()) {
        ...
        logger.reprintln('Loading task types... Done.');
    }
...
class PluginLoader {
    ...
    async _get(keyValue, pluginOption, pluginFns, logger) {
        ...
        if (existing) {
            ...
            new_.key = keyValue;
            setWith(this.loadedPlugins, [pluginOption.type, keyValue], new_, Object.constructor);
            return new_;
        }
    }
