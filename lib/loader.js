...
const {dedup, stableJSONStringify} = require('@raychee/utils');
...
class PluginLoader {
    ...
    constructor(loadPaths, storeCollection, defaultPlugins = {}, factoryConfigs = {}, options = {}) {
        this.loadPaths = loadPaths;
        ...
        this._get = dedup(
            PluginLoader.prototype._get.bind(this), 
            {key: (keyValue, pluginOption) => [pluginOption.type, keyValue], debug}
        );
    }
    ...
    async get(pluginOption, job) {
        ...
        plugin.refs++;
        debug('plugin get() for job %s: %j, refs = %s', get(job, 'config._id'), pluginOption, plugin.refs);
        if (plugin.ttl) {
            debug('plugin get() for job %s: %j, clear existing timeout', get(job, 'config._id'), pluginOption);
            clearTimeout(plugin.ttl);
            delete plugin.ttl;
        }
        return {
            ...plugin,
    ...
    async _get(keyValue, pluginOption, pluginFns, logger) {
        ...
        if (existing) {
            debug('plugin get() for job %s: %j, find existing', get(logger, 'config._id'), pluginOption);
            return existing;
        } else {
    ...
    async _create(pluginOption, key, {create, destroy, unload, config = {}}, logger) {
        debug('plugin get() for job %s: %j, create new', get(logger, 'config._id'), pluginOption);
        ...
        const created = {type, key, instance, logger: pluginLogger, config, refs: 0};
        const destroy_ = dedup(async () => {
            debug('plugin real destroy(): %j', pluginOption);
            if (key && this.loadedPlugins[type]) {
        ...
        created.unload = async (job) => {
            ...
            if (unload) {
                try {
                    ...
                    pluginLogger.warn('Plugin error during unload: ', e);
                }
            }
        };
        return created;
    ...
    async load() {
        ...
        for (let pluginModule of pluginModules) {
            if (pluginModule.factory) {
                pluginModule = {...pluginModule, ...await pluginModule.factory(this.factoryConfigs[pluginModule.type])};
            }
