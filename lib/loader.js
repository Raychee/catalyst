...
class TaskLoader {
     ...
     */
    async get(domainName, typeName) {
        return (await this.getAllTaskTypes())[`${domainName}.${typeName}`];
    }
    ...
    async load({verbose = false} = {}) {
        ...
        if (!this.taskAgenda && this.newAgenda) {
            ...
            this.taskAgenda.define('_task', async (agendaJob) => {
                ...
                const taskConfig = await this.operations.computeTask([taskId]);
    ...
    async syncConfigs() {
        const dataloaders = {};
        await this.operations.alignTaskDomainConfigs(dataloaders);
        await this.operations.alignTaskTypeConfigs(undefined, dataloaders);
    }
