...
const {limit, stableJSONStringify} = require('@raychee/utils');
...
class PluginLoader {
    ...
    async _create(pluginOption, key, {create, destroy, unload, config = {}}, logger) {
        ...
        return {
            key, instance, logger: pluginLogger, config,
            async unload(job) {
                if (unload) {
                    await unload.call(pluginLogger, instance, job)
                        .catch(e => pluginLogger.warn('Plugin error during unload: ', e));
                }
                if (destroy && (config.destroyOnUnload || !key)) {
                    await destroy.call(pluginLogger, instance)
                        .catch(e => pluginLogger.warn('Plugin error during destroy on unload: ', e));
                }
            },
            async destroy() {
                if (destroy) {
    ...
    async unload() {
        const promises = Object.values(this.loadedPlugins)
            .flatMap(v => Object.values(v).map(({destroy}) => destroy()));
