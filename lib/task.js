    ...
    async _load() {
        if (!this._jobLogger) {
            this._jobLogger = new JobLogger({
                prefixes: () => [this._taskType.fullName, this.config.task.toString(), this.config._id.toString()]
        });
        }
        if (!this._logger) {
            this._logger = new Logger({
                prefixes: () => [
                    this._taskType.fullName, this.config.task.toString(), this.config._id.toString(), this.config.params, this.config.context
                ],
            });
        }
    ...
    async _executeTrial() {
        try {
            ...
            if (e instanceof CatchableError) {
                try {
                    let catchMessage = await this._withCatchable(
                        this._taskType.catch.bind(
                            this._asAgent(), e.error, this.config.params, this.config.context, this._taskType.store
                        )
                    );
                    if (catchMessage) {
