...
class TaskDomain {
    ...
    async load({plugins, store, validate, dedup, catch: catch_, failed, final}) {
        if (this.pluginLoader) {
            ...
            this.plugins = merge(this.pluginLoader.defaultPlugins, await ensureThunkCall(plugins, logger) || {}, {inplace: false, depth: 1});
...
class TaskType {
    ...
    async load({plugins, store, validate, dedup, run, catch: catch_, failed, final}) {
        if (this.pluginLoader) {
            ...
            this.plugins = merge(this.domain.plugins, await ensureThunkCall(plugins, logger) || {}, {inplace: false, depth: 1});
            this.store = {
                ...this.domain.store,
...
class Job extends EventEmitter {
    ...
    constructor(config, taskType, scheduler, operations) {
        ...
        this._jobWatcher = new JobWatcher(this);
    ...
    async wait(job, {forSuccess = true} = {}) {
        ...
        job = await this._jobWatcher.watch(jobId);
...
class JobWatcher {
    ...
    _watch(jobId) {
        if (this._watches[jobId] && this.job._operations) {
            debug('job %s is watching job %s to stop', this.job.config._id, jobId);
            const watch = this.job._operations.jobs.watch(
                [{
                    $match: {
            ...
            watch
                ...
                .on('error', (error) => {
                    debug('job %s encounters an error watching job %s to stop: %s', this.job.config._id, jobId, error);
                    this._close(jobId);
                    if (this.job._isActive && !(error.name === 'MongoError' && error.code === 40573)) {
                        this._watch(jobId);
                    }
                });
    ...
    watched(jobId, value) {
        debug('job %s watches that job %s has stopped', this.job.config._id, jobId);
    ...
    abortAll(error) {
        debug('job %s aborts watching for jobs: %s', this.job.config._id, this.watchingJobIds.join(', '));
    ...
    closeAll() {
        debug('job %s closes watching for jobs: %s', this.job.config._id, this.watchingJobIds.join(', '));
