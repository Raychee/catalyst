...
class TaskDomain {
    ...
    async load({plugins, store, validate, dedup, catch: catch_, failed, final}) {
        if (this.pluginLoader) {
            ...
            this.plugins = merge(this.pluginLoader.defaultPlugins, await ensureThunkCall(plugins, logger), {depth: 1});
...
class TaskType {
    ...
    async load({plugins, store, validate, dedup, run, catch: catch_, failed, final}) {
        if (this.pluginLoader) {
            ...
            this.plugins = merge(this.domain.plugins, await ensureThunkCall(plugins, logger), {depth: 1});
            this.store = {
                ...this.domain.store,
