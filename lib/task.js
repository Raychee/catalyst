...
const {
    sleep, random, ensureThunkCall, merge2Level, dedup, limit, shrink
...
class TaskType {
     ...
     */
    constructor(domain, name, pluginLoader, storeCollection) {
        this.domain = domain;
        this.name = name;
        this.pluginLoader = pluginLoader;
        this.storeCollection = storeCollection;
...
class Job {
    ...
    async _delay(conf, {trial = 0, updateStatus = false} = {}) {
        ...
        if (delay > 0) {
            ...
            const heartbeat = this._scheduler ? this._scheduler.options.heartbeat * 1000 : Number.MAX_SAFE_INTEGER;
            while (true) {
    ...
    async _load() {
        if (!this._jobLogger) {
            this._jobLogger = new JobLogger(
                '', () => [this._taskType.fullName, this.config.task, this.config._id]
            );
        }
        if (!this._logger) {
            this._logger = new Logger(
                '', () => [
                    this._taskType.fullName, this.config.task, this.config._id, this.config.params, this.config.context
                ],
            );
        }
    ...
    async _updateTrial({status, delay, code, message, timeStopped}, {includeContext = false} = {}) {
        ...
        if (status) {
            ...
            if (status === 'RUNNING') {
                ...
                if (
                    ['SUCCESS', 'CANCELED'].includes(status) ||
    ...
    async _update(update, {includeContext = false} = {}) {
        ...
        if (this._operations) {
            ...
            if (this._scheduler && (!this._scheduler.isActive || lockedBy !== this._scheduler.id)) {
    ...
    async _syncStatus() {
        while (this._isActive) {
            if (this._scheduler) {
                await sleep(this._scheduler.options.heartbeat * 1000);
            }
            try {
                if (this.config._id && this._operations) {
                    await this._update();
    ...
    async _executeTrial() {
        try {
            ...
            await this._withCatchable(
                this._taskType.run.bind(this._asAgent(), this.config.params, this.config.context, this._taskType.store)
            );
            ...
            if (e instanceof CatchableError) {
                e = e.error;
                try {
                    let catchMessage = await this._taskType.catch.call(
                        this._asAgent(), e, this.config.params, this.config.context, this._taskType.store
                    );
                    if (catchMessage) {
            ...
            if (e instanceof JobRuntimeError) {
                ...
                await this._updateTrial(
                    {status: 'FAILED', code: e.code || '_crash', message: e.message, timeStopped: new Date()},
                    {includeContext: true}
                );
                throw e;
            }
