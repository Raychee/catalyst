...
class Job extends EventEmitter {
    ...
    async wait(job, {forSuccess = true, timeout = -1} = {}) {
        ...
        if (!(timeout >= 0) && this.config.timeout > 0) {
            ...
            timeout = Math.round((trial.timeStarted.getTime() + this.config.timeout * 1000 - Date.now()) / 1000);
        }
        while (true) {
            this._logger.wait(
    ...
    _checkStatusChange() {
        if (this._scheduler && !this._scheduler.isRunning) this._interrupted = true;
