...
class Logger {
    ...
    _log(level, log, tag, singleLine, ...values) {
        ...
        if (!this.lastMessageHasNewLine && singleLine) {
            n = '\n';
        }
        log(`${r}${timestamp}${tag}${prefixes}${n}${message}`);
        this.lastMessageHasNewLine = message.endsWith('\n');
    }
...
Logger.prototype.lastMessageHasNewLine = true;
