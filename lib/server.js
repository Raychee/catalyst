...
const {Runnable, randomString, truncate} = require('@raychee/utils');
...
module.exports = class extends Runnable {
    constructor(options) {
        ...
        this.options.server.host = this.options.server.host || '0.0.0.0';
        this.options.server.port = this.options.server.port || 4000;
        ...
        this.options.logging.truncate = this.options.logging.truncate || -1;
        ...
        Logger.prototype.TRUNCATE = this.options.logging.truncate;
    }
    ...
    async run({signal}) {
        ...
        const taskLoader = new TaskLoader(this.options.tasks.paths, undefined, this.options.tasks.loader);
        ...
        function truncate_(str) {
            if (this.options.logging.truncate > 0) {
                return truncate(str, {maxLength: this.options.logging.truncate});
            } else {
                return str;
            }
        }
        ...
        app.use(async (ctx, next) => {
            ...
            console.log(`(${id}) ${ctx.ips.join(' -> ')} -> ${ctx.method} ${ctx.originalUrl} ${JSON.stringify(mapValues(ctx.request.headers, truncate_))} ${truncate_(ctx.request.body)}`);
            const tick = Date.now();
            await next();
            const time = Date.now() - tick;
            console.log(`(${id}) ${ctx.ips.join(' <- ')} <-${time}ms- ${ctx.method} ${ctx.originalUrl} [${ctx.status}] ${truncate_(ctx.body)}`);
        });
        ...
        const server = app.listen(this.options.server.port, this.options.server.host, () => {
            this.logger.reprintln(
                this.options.name, ' (server) is ready at http://', this.options.server.host, 
                ':', this.options.server.port, apollo.graphqlPath
            );
        });
