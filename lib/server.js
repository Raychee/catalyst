...
const {setConfig, makeSchemaDirectives} = require('graphql-augment');
...
module.exports = class extends Runnable {
    ...
    async run({signal}) {
        ...
        const jobWatcher = new JobWatcher(operations.jobs, {options: {fullDocument: 'updateLookup'}});
        ...
        setConfig({
            FIELD_PREFIX_UPDATE: 'Undefined',
            FIELD_PREFIX_UPSERT: 'Update',
        });
        const apollo = new ApolloServer({
            typeDefs, resolvers: makeResolvers({auth: this.options.server.auth}),
            context: ({ctx}) => ({req: ctx.req, operations, jobWatcher}),
        ...
        jobWatcher.abortAll(new Error(`${this.options.name} (server) shutting down`));
        await mongodb.close();
