...
class Scheduler {
    ...
    async _clearDeadSchedulers() {
        const now = new Date();
        debug('clear dead scheduler at %s', now);
        ...
        await this.operations.updateJobs(
            {lockedBy: {$nin: [null, ...activeIds]}, status: {$in: ['DELAYED', 'RUNNING']}},
    ...
    async _dispatchJobs() {
        ...
        debug('dispatch jobs: running domain %o', runningByDomain);
        debug('dispatch jobs: running type %o', runningByType);
        ...
        debug('dispatch jobs: concurrency domain %o', concurrencyByDomain);
        debug('dispatch jobs: concurrency type %o', concurrencyByType);
