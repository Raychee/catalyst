...
const {Watcher, JobWatcher} = require('./watcher');
...
class Scheduler extends Runnable {
    ...
    constructor(logger, operations, taskLoader, options) {
        ...
        this.jobWatcher = new JobWatcher(this.operations.jobs, {
            targetStatus: ['SUCCESS', 'FAILED', 'CANCELED'],
            pollInterval: this.options.heartbeat,
        });
            ...
            while (true) {
                ...
                for await (const {_id, domain, type, task} of cursor) {
                    debug(
                        ...
                        'remaining domainConcurrency = %s, typeConcurrency = %s', 
