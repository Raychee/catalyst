...
const {HeartAttack} = require('./error');
...
class Scheduler {
    ...
    get isActive() {
        return this.id !== undefined && this.running;
    }
    ...
    async stop() {
        ...
        if (id) {
            ...
            await this.operations.schedulers.updateMany(
                {_id: id}, {$set: {heartbeat: null, timeStopped: now}}
            );
    ...
    async _dispatchJobs() {
        ...
        for await (const {domain, type, concurrency} of this.operations.taskTypes.find()) {
