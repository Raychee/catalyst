...
class Scheduler extends Runnable {
    ...
    constructor(logger, operations, taskLoader, options) {
        ...
        this.schedulingTasks = new ConcurrencyController(() => this._scheduleTasks().catch(
            e => this.logger.warn('Scheduling tasks failed in scheduler ', this.id, ': ', e)
        ), {concurrency: 2});
        this.dispatchingJobs = new ConcurrencyController(() => this._dispatchJobs().catch(
            e => this.logger.warn('Dispatching jobs failed in scheduler ', this.id, ': ', e)
        ), {concurrency: 2});
        this.schedulingJobs = new ConcurrencyController(() => this._scheduleJobs().catch(
    ...
    async run({signal}) {
        ...
        while (this.isRunning) {
            try {
                ...
                if (this.isRunning) await this.scheduleTasks({concurrency: 1});
                if (this.isRunning) await this.dispatchJobs({concurrency: 1});
                if (this.isRunning) await this.scheduleJobs({concurrency: 1});
            } catch (e) {
    ...
    async scheduleTasks(options) {
        return this.schedulingTasks.apply(options);
    }
    ...
    async dispatchJobs(options) {
        return this.dispatchingJobs.apply(options);
    }
    ...
    async scheduleJobs(options) {
        return this.schedulingJobs.apply(options);
    }
...
class ConcurrencyController {
    ...
    apply(options) {
        ...
        if (this.queue < options.concurrency) {
            const p = () => this.fn()
                .finally(() => {
                    this.queue--;
                    if (this.queue <= 0) {
