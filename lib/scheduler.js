...
class Scheduler extends Runnable {
    ...
    async _scheduleTasks() {
        ...
        for await (const task of this.operations.tasks.find(
            {lockedBy: this.id},
            {projection: {nextTime: 1, lastTime: 1, mode: 1, interval: 1, schedule: 1, timezone: 1}}
        )) {
            const now = new Date();
            ...
            await this.operations.updateTasks({_id, lockedBy: this.id}, {lastTime, nextTime, lockedBy: null});
        }
    }
