...
const {CatchableError, JobEarlyExit, HeartAttack} = require('./error');
...
class Scheduler extends Runnable {
    ...
    async _executeJob(config) {
        ...
        try {
            ...
            if (!(e instanceof JobEarlyExit) && !(e instanceof CatchableError)) {
    ...
    async _clearDeadSchedulers() {
        ...
        await this.operations.updateJobs(
            {lockedBy: {$nin: [null, ...activeIds]}, status: {$in: ['PENDING', 'CANCELED', 'FAILED', 'SUCCESS']}},
            {lockedBy: null},
        );
    }
